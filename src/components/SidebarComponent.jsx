...
import { getAllWorkspace } from "@/services/(workspace-service)/workspace-services";

export default async function SidebarComponent() {
 const workPlace = await getAllWorkspace();
 return (
    <div className="pl-10 mt-6 h-screen">
      ...
      {/* each workspace */}
      {workPlace.data.map((work) => (
        <div key={work.workSpaceId} className="flex items-center mt-5 w-full">
          <div className="rounded-full w-4 h-4 bg-todo"></div>
          <div className="flex justify-between w-full pl-3">
            <p>{work.workspaceName}</p>
            <EditDeleteDropDownComponent workspaceId = {work.workSpaceId}/>
          </div>
        </div>
      ))}
